...
function getSchemas(name?: string): RpcSchema[] {
  if (name) {
...
async function handleRpcMessage(
  ...
  if (message.action === "__rpc_schema") {
    return {
      success: true,
      schema: getSchemas(message.className || undefined),
    };
  }
...
function ensureListener() {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "__rpc_schema" && request.className) {
      if (!registry.has(request.className)) {
        return false; // schema requested for a different context
      }
    }
